...
class UserOutDoorController extends Controller
{
    ...
    public function handlePaycly(Request $request)
    {
        ...
        if($deposit) {
            if($data['status'] == 'Failed') {
                ...
                $respT7 = $this->performTransaction($t7->currency, $t7->number, $amount, 'GDP-Paycly', 'GDP-AUTOPAYCLY-'.$paymentId, 'deposit', 'balance');
    ...
    public function xproNotifications(Request $request)
    {
        ...
        if(strtolower($data['type'] == 'sale' && $data['status']) == 'success' && $deposit->status == 'Pending') {
            $respT7 = $this->performTransaction($data['order_currency'], $t7->number, $amount, 'GDP-Xpro', 'GDP-AUTOXPRO-'.$txn_id, 'deposit', 'balance');
